...
class FSLQuery(nn.Module):
    ...
    def forward_PyramidFCN(self, support_x, support_y, query_x, query_y):
        ...
        if isinstance(support_xf, list): support_xf = support_xf[-1]
    ...
    def forward_FCN_R12(self, support_x, support_y, query_x, query_y):
        is_5d = support_x.dim() == 5
        if is_5d:
            b, s, c, h, w = support_x.size()
            b, q, _, _, _ = query_x.size()
            support_x = support_x.view(b*s, c, h, w)
            query_x = query_x.view(b*q, c, h, w)
        support_xf_list = self.encoder(support_x)
        query_xf_list = self.encoder(query_x)
        if not isinstance(support_xf_list, list):
            support_xf_list = [support_xf_list]
            query_xf_list = [query_xf_list]
        if hasattr(self, 'ms_fusion'):
            _vc = []
            for c_block in self.ms_fusion.align_convs:
                for m in c_block.modules():
                    if hasattr(m, 'in_channels'):
                        _vc.append(m.in_channels)
                        break
            _s_feats = [f for f in support_xf_list if f.size(1) in _vc] or support_xf_list[-len(self.ms_fusion.align_convs):]
            support_xf = self.ms_fusion(_s_feats)
            _q_feats = [f for f in query_xf_list if f.size(1) in _vc] or query_xf_list[-len(self.ms_fusion.align_convs):]
            query_xf = self.ms_fusion(_q_feats)
        else:
            support_xf, query_xf = support_xf_list[-1], query_xf_list[-1]
        if is_5d:
            support_xf = support_xf.view(b, s, support_xf.size(1), support_xf.size(2), support_xf.size(3))
            query_xf = query_xf.view(b, q, query_xf.size(1), query_xf.size(2), query_xf.size(3))
        return support_xf, support_y, query_xf, query_y
    ...
    def forward_feature(self, support_x, support_y, query_x, query_y):
        if self.forward_encoding == "FCN":
            ...
            support_xf, support_y, query_xf, query_y = self.forward_FCN_R12(support_x, support_y, query_x, query_y)
