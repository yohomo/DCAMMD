...
class BasicBlock(nn.Module):
    expansion = 1
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.0, drop_block=False, block_size=1, norm_layer='vanilla'):
...
class ResNet(nn.Module):
    def __init__(self, block=BasicBlock, drop_rate=0.0, dropblock_size=3, norm_layer='vanilla'):
        self.inplanes = 3
        super(ResNet, self).__init__()
        self._norm_layer = get_batchnorm_2d(norm_layer)
    ...
    def _make_layer(self, block, planes, stride=1, drop_rate=0.0, drop_block=False, block_size=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, drop_rate, drop_block, block_size, norm_layer=self._norm_layer))
...
def resnet12():
    return ResNet(BasicBlock, norm_layer='vanilla')

@registry.Encoder.register("R12-pre")
def resnet12_pre():
    return ResNet(BasicBlock, drop_rate=0.1, dropblock_size=5, norm_layer='vanilla')
